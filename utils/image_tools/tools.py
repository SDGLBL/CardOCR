...
def random_paste_image(image_list, target_size, rgb):
    r, g, b = rgb
    union_img_np = np.concatenate(image_list, axis=1)
    union_img = Image.fromarray(union_img_np, 'RGB')
    iw, ih = union_img.size
    w, h = target_size
    new_backround = Image.new('RGB', target_size, (r, g, b))
    random_move = random.randint(-30, 30)
    new_backround.paste(union_img, (((w - iw) // 2) + random_move, (h - ih) // 2))
    return new_backround


def random_connect_image(image1, image2, connect_imag):
    np1 = np.array(image1)
    np2 = np.array(image2)
    np3 = np.array(connect_imag)
    one_img = np.concatenate([np1, np3, np2], axis=1)
    # 随机打乱
    union_img_array = [np3, one_img, np3, one_img, np3]
    np.random.shuffle(union_img_array)
    union_img = np.concatenate(union_img_array, axis=1)
    return union_img
...
# 改变指定文件夹内所有图像为指定格式
def process(target_floder, des_floder, target_type):
    image_list = pathlib.Path(target_floder).glob('*.*')
    for path in image_list:
        image = Image.open(path).convert('RGB')
        name = path.stem
        image.save(os.path.join(des_floder, name + '.' + target_type))
    print('转换完成')


# 获得指定文件夹内指定文件类型数目（默认全部匹配）
def get_num_of_folder(path, pattern='*'):
    return len(list(pathlib.Path(path).glob(pattern)))
...
class DataAugmentation:
    ...
    def randomRotation(image, mode=Image.BICUBIC):
        ...
        :return: 旋转转之后的图像
        """
        random_angle = np.random.randint(1, 6)
    ...
    # //TODO(lijie) The size of image need same as what size you want to train
    @staticmethod
    def saveImage(image, path):
        image = image.resize((256, 60), Image.ANTIALIAS)
        image.save(path)
